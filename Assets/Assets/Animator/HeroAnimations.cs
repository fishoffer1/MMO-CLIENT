...
public class HeroAnimations : MonoBehaviour
{
    ...
    {
        int eid = entitySync.Entity.Id;
        if (eid != gameEntity.entityId) return;
    ...
    public AnimancerState Play(string name, Action OnEnd=null)
    {
        if (_animancer == null)
            return null;
        var state = _animancer.TryPlay(name);
        if(state == null)
        {
    ...
    void Update()
    {
        ...
        {
            //施法动作
            if (skill.State == Skill.Stage.Intonate)
            {
                Play(skill.Def.Anim1);
            }
            else if (skill.State == Skill.Stage.Active)
            {
                Play(skill.Def.Anim2);
            }
            //恢复动作
            if (state == HState.SkillIntonate && skill.State != Skill.Stage.Intonate)
            {
                state = HState.Idle;
                PlayIdle();
            }
            if (state == HState.SkillActive && skill.State != Skill.Stage.Active)
            {
                state = HState.Idle;
                PlayIdle();
            }
